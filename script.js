...
dialog.addEventListener("click", function (e) {
  if (pagesInput.value != "" && pagesInput.value < 1) pagesInput.value = 1;
});

addBook.addEventListener("click", function (e) {
  const error = document.querySelector(".error");
  if (titleInput.value.trim() === "") {
    error.innerText = "Title field must be filled.";
    return;
  }
  if (authorInput.value.trim() === "") {
    error.innerText = "Author field must be filled.";
    return;
  }
  if (pagesInput.value.trim() === "") {
    error.innerText = "Pages field must be filled.";
    return;
  }
